...
import { ArrowDown, ArrowUp, Bitcoin, Search, SlidersHorizontal, Star } from "lucide-react"
...
import type { CryptoMarketProps } from "@/app/utils/market-types"
      ...
      <CardHeader className="pb-2 sm:pb-3">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 sm:gap-4">
          <div>
            ...
            <CardDescription className="text-xs sm:text-sm">Real-time cryptocurrency market data</CardDescription>
          </div>
              ...
              {sortedData.length > 0 ? (
                sortedData.map((crypto) => (
                  <TableRow key={crypto.symbol} className="hover:bg-muted/50 transition-colors">
                    <TableCell className="font-medium text-xs sm:text-sm py-1 sm:py-2 px-2 sm:px-4">{crypto.symbol}</TableCell>
                    <TableCell className="text-xs sm:text-sm py-1 sm:py-2 px-2 sm:px-4 hidden sm:table-cell">{crypto.name}</TableCell>
                    <TableCell className="text-right font-medium text-xs sm:text-sm py-1 sm:py-2 px-2 sm:px-4">{formatCurrency(crypto.price)}</TableCell>
                    <TableCell
                      className={`text-right font-medium text-xs sm:text-sm py-1 sm:py-2 px-2 sm:px-4 ${
                        crypto.change_percentage > 0
                          ? "text-green-500"
                          : crypto.change_percentage < 0
                            ? "text-red-500"
                            : ""
                    ...
                    <TableCell className="text-right font-mono text-xs py-1 sm:py-2 px-2 sm:px-4 hidden md:table-cell">{formatCurrency(crypto.market_cap)}</TableCell>
                    <TableCell className="text-right font-mono text-xs py-1 sm:py-2 px-2 sm:px-4 hidden md:table-cell">{formatCurrency(crypto.volume)}</TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-6 sm:py-8">
                    <div className="flex flex-col items-center justify-center text-muted-foreground">
                      ...
                      <p className="text-xs sm:text-sm">No cryptocurrencies found</p>
