...
import { ArrowDown, ArrowUp, DollarSign, Search, SlidersHorizontal, Star } from "lucide-react"
import { formatPercentage } from "@/app/utils/format-utils"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import type { ForexMarketProps } from "@/app/utils/market-types"

export default function ForexMarket({ data }: ForexMarketProps) {
  ...
  const filteredData = data.filter(
    (forex) =>
      forex.pair.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <CardHeader className="pb-2 sm:pb-3">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 sm:gap-4">
          <div>
            ...
            <CardDescription className="text-xs sm:text-sm">Real-time foreign exchange rates</CardDescription>
          </div>
          <div className="flex items-center gap-2 mt-2 md:mt-0">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="text-xs sm:text-sm">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => handleSort("pair")}>Pair</DropdownMenuItem>
      ...
      <CardContent className="px-1 sm:px-3 md:px-6">
        <div className="rounded-md border overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {sortedData.length > 0 ? (
                sortedData.map((forex) => (
                  <TableRow key={forex.pair} className="hover:bg-muted/50 transition-colors">
                    <TableCell className="font-medium text-xs sm:text-sm py-1 sm:py-2 px-2 sm:px-4">{forex.pair}</TableCell>
                    <TableCell className="text-xs sm:text-sm py-1 sm:py-2 px-2 sm:px-4 hidden sm:table-cell">{forex.name}</TableCell>
                    <TableCell className="text-right font-mono text-xs sm:text-sm py-1 sm:py-2 px-2 sm:px-4">{forex.rate.toFixed(4)}</TableCell>
                    <TableCell
                      className={`text-right font-medium text-xs sm:text-sm py-1 sm:py-2 px-2 sm:px-4 ${
                        forex.change_percentage > 0
                          ? "text-green-500"
                          : forex.change_percentage < 0
                            ? "text-red-500"
                            : ""
                      ...
                      <div className="flex items-center justify-end">
                        {forex.change_percentage > 0 ? (
                          <div className="bg-green-100 text-green-700 px-1 sm:px-2 py-0.5 sm:py-1 rounded-md flex items-center text-xs">
                            ...
                            {formatPercentage(forex.change_percentage)}
                          </div>
                        ) : forex.change_percentage < 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-6 sm:py-8">
                    <div className="flex flex-col items-center justify-center text-muted-foreground">
                      ...
                      <p className="text-xs sm:text-sm">No forex pairs found</p>
